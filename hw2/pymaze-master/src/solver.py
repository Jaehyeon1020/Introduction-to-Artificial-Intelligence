...
from q_learning import q_learning_path
...
class BreadthFirst(Solver):
    ...
    def solve(self):
        ...
        # Stack of cells at current level of search
        ...
        while True:  # Loop until return statement is encountered
            ...
            while current_level:  # While still cells left to search on current level
                ...
                # Append current cell to total search path
                path.append(((k_curr, l_curr), False))

                if (k_curr, l_curr) == self.maze.exit_coor:  # Exit if current cell is exit cell
                    if not self.quiet_mode:
                        print("Number of moves performed: {}".format(len(path)))
                        # print("Execution time for algorithm: {:.4f}".format(
                        #     time.clock() - time_start))
                    return path

                neighbour_coors = self.maze.find_neighbours(
                    k_curr, l_curr)  # Find neighbour indicies
                ...
                if neighbour_coors is not None:
                    for coor in neighbour_coors:
                        # Add all existing real neighbours to next search level
                        next_level.append(coor)

            for cell in next_level:
                # Update current_level list with cells for nex search level
...
class BiDirectional(Solver):
    ...
    def solve(self):
        ...
        grid = self.maze.grid
        # Where to start the first search
        k_curr, l_curr = self.maze.entry_coor
        # Where to start the second search
        ...
        while True:   # Loop until return statement is encountered
            neighbours_kl = self.maze.find_neighbours(
                k_curr, l_curr)    # Find neighbours for first search
            real_neighbours_kl = [neigh for neigh in neighbours_kl if not grid[k_curr]
                                  [l_curr].is_walls_between(grid[neigh[0]][neigh[1]])]
            neighbours_kl = [
                neigh for neigh in real_neighbours_kl if not grid[neigh[0]][neigh[1]].visited]

            neighbours_pq = self.maze.find_neighbours(
                p_curr, q_curr)    # Find neighbours for second search
            real_neighbours_pq = [neigh for neigh in neighbours_pq if not grid[p_curr]
                                  [q_curr].is_walls_between(grid[neigh[0]][neigh[1]])]
            neighbours_pq = [
                neigh for neigh in real_neighbours_pq if not grid[neigh[0]][neigh[1]].visited]
            ...
            if len(neighbours_kl) > 0:   # If there are unvisited neighbour cells
                ...
                k_next, l_next = random.choice(
                    neighbours_kl)      # Choose random neighbour
            ...
            if len(neighbours_pq) > 0:
                ...
                p_next, q_next = random.choice(
                    neighbours_pq)   # Choose random neighbour
                # Move to that neighbour
            ...
            if any((True for n_kl in real_neighbours_kl if (n_kl, False) in path_pq)):
                path_kl.append(((k_curr, l_curr), False))
                path = [p_el for p_tuple in zip(
                    path_kl, path_pq) for p_el in p_tuple]  # Zip paths
                if not self.quiet_mode:
                    print("Number of moves performed: {}".format(len(path)))
                    # print("Execution time for algorithm: {:.4f}".format(
                    #     time.clock() - time_start))
...
class DepthFirstBacktracker(Solver):
    ...
    def solve(self):
        ...
        while (k_curr, l_curr) != self.maze.exit_coor:
            neighbour_indices = self.maze.find_neighbours(
                k_curr, l_curr)    # Find neighbour indices
            ...
            if neighbour_indices is not None:   # If there are unvisited neighbour cells
                ...
                path.append(((k_curr, l_curr), False))
                k_next, l_next = random.choice(
                    neighbour_indices)   # Choose random neighbour
                # Move to that neighbour
...
class QLearning(Solver):
    """A solver that implements the q-learning algorithm.
    """

    def __init__(self, maze, quiet_mode=False,  neighbor_method="fancy"):
        logging.debug('Class QLearning ctor called')

        super().__init__(maze, neighbor_method, quiet_mode)
        self.name = "QLearning"

    def solve(self):
        logging.debug("Class QLearning solve called")
        k_curr, l_curr = self.maze.entry_coor      # Where to start searching
        # Set initial cell to visited
        self.maze.grid[k_curr][l_curr].visited = True
        visited_cells = list()                  # Stack of visited cells for backtracking
        ...
        logging.debug('Class QLearning leaving solve')
        return path
